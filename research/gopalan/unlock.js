(() => {
  ...
  async function unlock(raw) {
    ...
    try {
      ...
      const clear = await crypto.subtle.decrypt({name:'AES-GCM',iv:decode(bundle.iv),additionalData:new TextEncoder().encode('gopalan-review:v1'),tagLength:128},key,decode(bundle.ciphertext));
